  ...
  return (dispatch) => {
    axios({
      ...
      .then((resp) => {
        ...
        axios
          .post('/minhasVacinas.json', {...minhasVacinas})
          .catch((err) => console.log(err))
  ...
  return (dispatch) => {
    if (minhasVacinas.imagem.base64 != null) {
      axios({
        ...
        .then((respo) => {
          axios
            .get(`/minhasVacinas/${minhasVacinas.id}.json`)
            .catch((err) => console.log(err))
            .then((resp) => {
              ...
              axios
                .patch(`/minhasVacinas/${minhasVacinas.id}.json`, {
                  imagem,
                  nome,
                  texto,
                  dtAplicacao,
...
export const delMinhasVacinas = (minhasVacinas) => {
  return (dispatch) => {
    axios
      .delete(`/minhasVacinas/${minhasVacinas.minhasVacinasId}.json`)
      .catch((err) => console.log(err))
      .then((res) => dispatch(fetchMinhasVacinas()));
  };
};
