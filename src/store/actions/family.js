    ...
    if (familia.imagem.base64 != null) {
        ...
        .then((respo) => {
          axios
            ...
            .then((resp) => {
              ...
              axios
                .patch(`/familias/${familia.id}.json`, {
                  imagem,
                  nomeFamiliar,
                  dtNascimento,
                })
      ...
      axios
        ...
        .then((resp) => {
          ...
          const dtNascimento = familia.dtNascimento;
          axios
            .patch(`/familias/${familia.id}.json`, {nomeFamiliar, dtNascimento})
            .catch((err) => console.log(err))
            .then((res) => {
              dispatch(fetchFamilia());
            });
...
export const delFamilia = (familia) => {
  return (dispatch) => {
    axios
      .delete(`/familias/${familia.familiaId}.json`)
      .catch((err) => console.log(err))
      .then((res) => {
        dispatch(fetchFamilia());
      });
...
export const addVacina = (vacina) => {
  return (dispatch) => {
    axios
      .get(`/familias/${vacina.vacinaId}.json`)
      .catch((err) => console.log(err))
      .then((res) => {
        const vacinas = res.data.vacinas || [];
        vacinas.push(vacina.vacina);
        axios
          .patch(`/familias/${vacina.vacinaId}.json`, {vacinas})
          .catch((err) => console.log(err))
          .then((res) => {
            dispatch(fetchFamilia());
          });
