...
class ListMinhasVacinas extends Component {
  ...
  render(){
    return (
      ...
      <FlatList  data={this.props.minhasVacinas.filter(x => x.userId === this.props.id)}
                keyExtractor={item => `${item.id}`}
                renderItem={({item}) => <MinhasVacina key={item.id} {...item} userId={item.id} minhasVacinasId={item.id} minhasVacinasEdt={item}/>} />
      </View>
      <View>
        <Menu/>
      </View>
